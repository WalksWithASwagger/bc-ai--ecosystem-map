...
/**
 * Simplified Phase 4 Import - Import new companies from Phase 4 research
 * Import new companies to approach legendary 1,000 company milestone
 */
...
class SimplifiedPhase4Importer {
  constructor() {
    ...
    try {
      ...
      if (company.focusAreas && company.focusAreas.length > 0) {
        properties['Focus & Notes'] = {
          rich_text: [{ text: { content: `Phase 4 - ${company.focusAreas.join(', ')} - ${company.category}` } }]
        };
      }

      if (company.employees) {
        properties['Company Size'] = { rich_text: [{ text: { content: company.employees } }] };
      }
  ...
  async run() {
    try {
      console.log('🚀 Phase 4 Import Starting...');
      ...
      await fs.writeFile(
        `logs/phase4-research/simplified-import-results-${new Date().toISOString().split('T')[0]}.json`,
...
if (require.main === module) {
  const importer = new SimplifiedPhase4Importer();
  importer.run().then(() => {
    console.log('✅ Phase 4 import completed!');
    process.exit(0);
  }).catch(error => {
    console.error('❌ Import failed:', error);
    process.exit(1);
