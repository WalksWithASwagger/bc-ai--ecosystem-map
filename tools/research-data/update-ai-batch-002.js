#!/usr/bin/env node
/**
 * Update AI companies batch 002 with deep people research
 */
const fs = require('fs');
...
async function updateBatch2() {
  console.log('🚀 Updating AI companies batch 002 with deep people data...\n');
  
  // Load the formatted updates
  const updates = require('./2025-01-30-ai-batch-002-formatted.json');
  ...
  for (const update of updates) {
    try {
      ...
      results.details.push({
        ...
        peopleAdded: (update.keyPeople.match(/\n/g) || []).length + 1
      });
  ...
  // Count total people added
  const totalPeople = results.details
    .filter(d => d.status === 'success')
    .reduce((sum, d) => sum + (d.peopleAdded || 0), 0);
  
  console.log(`👥 Total key people added: ${totalPeople}`);
  
  // Save results
  const timestamp = new Date().toISOString();
  const resultsPath = path.join(__dirname, `ai-batch-002-results-${timestamp.split('T')[0]}.json`);
  fs.writeFileSync(resultsPath, JSON.stringify(results, null, 2));
  console.log(`\n📄 Results saved to: ${resultsPath}`);
}

// Run the update
updateBatch2().catch(error => {
