#!/usr/bin/env node
/**
 * Update AI companies batch 001 with research data
 */
const fs = require('fs');
...
async function updateBatch() {
  console.log('🚀 Updating AI companies batch 001...\n');
  
  // Load the formatted updates
  const updates = require('./2025-01-30-ai-batch-001-formatted.json');
  ...
  const resultsPath = path.join(__dirname, `ai-batch-001-results-${timestamp.split('T')[0]}.json`);
  fs.writeFileSync(resultsPath, JSON.stringify(results, null, 2));
  console.log(`\n📄 Results saved to: ${resultsPath}`);
}

// Run the update
updateBatch().catch(error => {
