#!/usr/bin/env node
/**
 * Find websites for startups and scale-ups - Batch 3
 */
const fs = require('fs');
...
async function processStartups() {
  ...
  // Skip first 60 (already processed) and process next 30
  const startIndex = 60;
  const endIndex = Math.min(90, startups.length);
  ...
  const reportPath = path.join(__dirname, '..', 'reports', `${timestamp}_startup-website-enhancement-batch3.md`);
  
  let report = `# Startup Website Enhancement Report - Batch 3

*Generated on ${new Date().toLocaleString()}*

## Summary

- **Startups Processed**: ${results.processed} (items 61-90)
...
This is batch 3 of startup website enhancement (items 61-90). The script tested common domain patterns and variations of company names.
`;
