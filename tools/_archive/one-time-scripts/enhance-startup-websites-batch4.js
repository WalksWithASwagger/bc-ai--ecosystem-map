#!/usr/bin/env node
/**
 * Find websites for startups and scale-ups - Batch 4
 */
const fs = require('fs');
...
async function processStartups() {
  ...
  // Skip first 90 (already processed) and process next 30
  const startIndex = 90;
  const endIndex = Math.min(120, startups.length);
  ...
  const reportPath = path.join(__dirname, '..', 'reports', `${timestamp}_startup-website-enhancement-batch4.md`);
  
  let report = `# Startup Website Enhancement Report - Batch 4

*Generated on ${new Date().toLocaleString()}*

## Summary

- **Startups Processed**: ${results.processed} (items 91-120)
...
This is batch 4 of startup website enhancement (items 91-120). The script tested common domain patterns and variations of company names.
`;
